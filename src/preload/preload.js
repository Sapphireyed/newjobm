...
import { localization, wikiWords } from '../local/local.js'

let preload = document.createElement('div')
export {preload}

localization.wiki().then(loc=> {
    let loadingName, skipName, lang
    ...
    switch (lang) {
        case 'English':
            loadingName = wikiWords.filter(word => word[0] == 'Loading')[0][1]
            skipName = wikiWords.filter(word => word[0] == 'Skip')[0][2]
            break;
    }
    ...
    loading.innerHTML = loadingName

    let skip = document.createElement('a')
    skip.innerHTML = skipName
    skip.className = 'skip'
    ...
    preload.append(preloaddiv, loading, skip, skip2, chardiv)
